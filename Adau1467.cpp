...
void Adau1467::processWriteRequest(const TcpMessage& message) {
  ...
  if (header.dataLength > availableData) {
    Serial.println("Write Request: DataLength exceeds payload size");
    return;
  }
  if (header.dataLength > MAX_DATA_SIZE) {
    Serial.println("Write Request is too large");
    return;
  }
  const uint8_t* payload = message.data + WRITE_HEADER_SIZE;
  bool success = writeSpi(header.address, payload, header.dataLength);
}

void Adau1467::processReadRequest(const TcpMessage& message) {
  ReadRequestHeader request{};
  if (!decodeReadRequestHeader(message, request)) {
    Serial.println("Invalid Read Request");
    return;
  }
  if (request.dataLength > MAX_DATA_SIZE) {
    Serial.println("Read Request is too large");
    return;
  }
  ...
  response.data[12] = success ? 0x00 : 0x01;
  if (!success) memset(responsePayload, 0, request.dataLength);
  bool queued = _transmitQueue.send(response, pdMS_TO_TICKS(1000));
  if (!queued) {
...
bool Adau1467::writeSpi(uint16_t address, const uint8_t* data, uint32_t length) {
  ...
  esp_err_t result = spi_device_acquire_bus(_spiDevice, portMAX_DELAY);
  if (result != ESP_OK) {
    Serial.printf("Failed to acquire SPI bus: %s\n", esp_err_to_name(result));
    return false;
  }
  uint32_t offset = 0;
  ...
  while (offset < length) {
    ...
    size_t chunkLength = std::min(maximumPayload, remainingBytes);
    if (firstBlock) {
...
bool Adau1467::transmitRaw(const uint8_t* transmitData, uint8_t* receiveData, size_t length, bool keepChipSelectActive) {
  ...
  esp_err_t result = spi_device_polling_transmit(_spiDevice, &transaction);
  if (result != ESP_OK) {
    Serial.printf("SPI transfer failed: %s\n", esp_err_to_name(result));
    return false;
  }
  return true;
}
...
bool Adau1467::decodeWriteHeader(const TcpMessage& message, WriteRequestHeader& header) const {
  if (message.data == nullptr || message.length < WRITE_HEADER_SIZE) return false;
  if (message.data[0] != CONTROL_WRITE_REQUEST) return false;
  ...
  if (header.totalLength != message.length) return false;
  if (header.dataLength > message.length - WRITE_HEADER_SIZE) return false;
  return true;
}
bool Adau1467::decodeReadRequestHeader(const TcpMessage& message, ReadRequestHeader& header) const {
  if (message.data == nullptr || message.length < READ_REQUEST_SIZE) return false;
  if (message.data[0] != CONTROL_READ_REQUEST) return false;
  header.control = message.data[0];
